package com.jeckliu.multimedia.view;
...
public class ShootIconView extends View {
    ...
    public ShootIconView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (value == R.styleable.ShootIconView_outerPaintColor) {
                outerPaintColor = a.getColor(value, Color.GRAY);

            } else if (value == R.styleable.ShootIconView_outerProgressPaintColor) {
                outerProgressPaintColor = a.getColor(value, Color.BLUE);

            } else if (value == R.styleable.ShootIconView_innerPaintColor) {
                innerPaintColor = a.getColor(value, Color.WHITE);

            } else if (value == R.styleable.ShootIconView_outerOriginalRadius) {
                outerOriginalRadius = a.getDimension(value, 100);

            } else if (value == R.styleable.ShootIconView_innerOriginalRadius) {
                innerOriginalRadius = a.getDimension(value, 70);

            } else if (value == R.styleable.ShootIconView_innerScaleRadius) {
                innerScaleRadius = a.getDimension(value, 70);

            } else if (value == R.styleable.ShootIconView_outerScaleRadius) {
                outerScaleRadius = a.getDimension(value, 100);

            } else if (value == R.styleable.ShootIconView_progressWidth) {
                progressWidth = a.getDimension(value, 20);

            } else if (value == R.styleable.ShootIconView_shootMaxTime) {
